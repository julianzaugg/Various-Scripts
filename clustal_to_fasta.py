...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='convert clustal file to fasta file')
    ...
    parser.add_argument('-rg', '--remove_gaps', help='remove_gaps', required=False, default="no", 
        choices=["yes", "no"])
    args = parser.parse_args()
    the_aln = read_clustal_file(args.input)
    if args.remove_gaps == "yes":
        for s in the_aln.seqs:
            s.sequence = s.sequence.replace("-","")
    write_fasta_file(args.output, the_aln.seqs)
